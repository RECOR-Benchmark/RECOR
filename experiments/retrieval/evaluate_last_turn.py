"""
Last Turn Evaluation with Previous Turn Context

This script evaluates retrieval using previous turn's query + answer as context:
  - Turn 1: Original query only
  - Turn N: "Previous Question: ... Previous Answer: ... Current Question: ..."
...
def evaluate_domain_conversational(domain, dataset_dir, args, config):
    ...
    if conversations:
        ...
        if 'turns' in first_conv and first_conv['turns']:
            ...
            if 'answer' in first_turn:
                print(f"  Answer field found: 'answer'")
            elif 'response' in first_turn:
                print(f"  Answer field found: 'response'")
            else:
                print(f"  WARNING: No answer field found!")

    # Prepare all queries from all turns (so model is loaded only once)
    # ABLATION: Include previous turn's query and answer for context
    all_queries = []
    all_query_ids = []
    gold_ids_map = {}
    excluded_ids = {}
    ...
    print("\n[ABLATION MODE] Augmenting queries with previous turn context")
    print("  Turn 1: Original query only")
    print("  Turn N: Previous Query + Previous Answer + Current Query")
    ...
    for conv in conversations:
        ...
        for turn_idx, turn in enumerate(turns):
            ...
            # ABLATION STUDY: Augment query with previous turn context
            if turn_idx == 0:
                # Turn 1: No previous context
                augmented_query = turn['query']
            else:
                # Turn N (N>1): Include previous turn's query + answer
                prev_turn = turns[turn_idx - 1]
                prev_query = prev_turn['query']
                # Try different field names for answer
                prev_answer = prev_turn.get('answer', prev_turn.get('response', prev_turn.get('gold_answer', '')))

                # Format: Previous context + Current query
                augmented_query = f"Previous Question: {prev_query}\nPrevious Answer: {prev_answer}\nCurrent Question: {turn['query']}"

            all_queries.append(augmented_query)
            all_query_ids.append(query_id)

            # Handle both possible field names for gold documents
            if 'gold_doc_ids' in turn:
                gold_ids_map[query_id] = turn['gold_doc_ids']
            elif 'supporting_doc_ids' in turn:
                gold_ids_map[query_id] = turn['supporting_doc_ids']
            else:
                print(f"Warning: No gold IDs found for {query_id}, available keys: {turn.keys()}")
                gold_ids_map[query_id] = []
    ...
    # Show example of augmented query
    if len(all_queries) > 1:
        print(f"\n  Example augmented query (turn 2):")
        print(f"  ---")
        print(f"  {all_queries[1][:500]}..." if len(all_queries[1]) > 500 else f"  {all_queries[1]}")
        print(f"  ---")
    ...
    for conv_id in conversation_results:
        turn_results_list = [
    ...
    detailed_results = {
        ...
        'ablation_mode': 'previous_turn_context',
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_dir', type=str, default='outputs_conversational_ablation')
    ...
    # Create main output directory - note the ablation suffix
    args.output_dir = os.path.join(args.output_dir, f"conversational_ir_{args.model}_prev_turn_context")
    ...
    print("\n" + "="*80)
    print("AGGREGATED RESULTS ACROSS ALL DOMAINS (ABLATION: Previous Turn Context)")
    print("="*80)
    ...
    if len(all_domain_results) > 0:
        ...
        summary = {
            'model': args.model,
            'ablation_mode': 'previous_turn_context',
            'description': 'Turn N query augmented with Turn N-1 query and answer',
        ...
        print(f"\n Ablation evaluation complete! Results saved to {args.output_dir}")
